...
float4 bilinear(float2 coord)
{
    ...
    float2 pos = coord + size * 0.5;
    float2 f = frac(pos / size);

    float tli = tex2D(SurfaceTex, pos).r;
    float tri = tex2D(SurfaceTex, pos + float2(size.x, 0.0)   ).r;
    float bli = tex2D(SurfaceTex, pos + float2(0.0,    size.y)).r;
    float bri = tex2D(SurfaceTex, pos + float2(size.x, size.y)).r;
